	...
	try {
		...
		if (process.platform === "win32") {
			...
			const version = fs
				.readdirSync(basedir)
				.filter(
					(f) => safeIsDir(path.join(basedir, f)) && f.split(".").length > 1,
				)
				.sort()
				.reverse()[0];
			if (!version) return "";
			...
			const basedir = path.join(
				...
				"..",
				releaseChannel.toLowerCase().replace(" ", ""),
			);
			if (!fs.existsSync(basedir)) return "";
			const version = fs
				.readdirSync(basedir)
...
export const validatePath = function (channel, proposedPath) {
	if (process.platform === "win32")
		return validateWindows(channel, proposedPath);
...
const validateWindows = function (channel, proposedPath) {
	...
	if (isBaseDir) {
		const version = fs
			...
			.filter(
				(f) => safeIsDir(path.join(proposedPath, f)) && f.split(".").length > 1,
			)
			.sort()
			.reverse()[0];
		if (!version) return "";
		// To account for discord_desktop_core-1 or discord_dekstop_core-2
		const coreWrap = fs
			.readdirSync(path.join(proposedPath, version, "modules"))
			.filter((e) => e.indexOf("discord_desktop_core") === 0)
			.sort()
			.reverse()[0];
		corePath = path.join(
	...
	if (selected.split(".").length > 2) {
		// To account for discord_desktop_core-1 or discord_dekstop_core-2
		const coreWrap = fs
			.readdirSync(path.join(proposedPath), "modules")
			.filter((e) => e.indexOf("discord_desktop_core") === 0)
			.sort()
			.reverse()[0];
		corePath = path.join(
...
const validateLinuxMac = function (channel, proposedPath) {
	if (proposedPath.includes("/snap/")) {
		remote.dialog.showErrorBox(
			"BetterDiscord Incompatible",
			"BetterDiscord is currently incompatible with Snap installs of Discord. Support for snap installs is coming soon!",
		);
		return "";
	}
	...
	if (selected.split(".").length > 2)
		resourcePath = path.join(proposedPath, "modules", "discord_desktop_core");
	if (selected === "modules")
		resourcePath = path.join(proposedPath, "discord_desktop_core");
